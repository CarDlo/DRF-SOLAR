...
def start_modbus_client(plant_name,ip, port, start_address, max_registers, interval, modelo):
    ...
    def send_data_to_api(value, reg_address):
        ...
        try:
            match modelo:
                case "Bayunca1":
                    Bayunca.objects.create(REG_CA=reg_address, value=value)
                case "Lavilla":
                    LaVilla.objects.create(REG_CA=reg_address, value=value)
